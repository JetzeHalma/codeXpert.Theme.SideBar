...
AdminLte.SideBar = {
    load: (function (global, factory) {
        ...
        class Layout {
            ...
            holdTransition() {
                ...
                window.addEventListener('resize', () => {
                    themebody.classList.add(CLASS_NAME_HOLD_TRANSITIONS);
                    clearTimeout(resizeTimer);
                    resizeTimer = setTimeout(() => {
                        themebody.classList.remove(CLASS_NAME_HOLD_TRANSITIONS);
                    }, 400);
                });
            }
        }
        onDOMContentLoaded(() => {
            const themebody = document.getElementById('themeBody');
            const data = new Layout(themebody);
            data.holdTransition();
            setTimeout(() => {
                themebody.classList.add(CLASS_NAME_APP_LOADED);
            }, 400);
        });
        ...
        class PushMenu {
            ...
            expand() {
                ...
                const themebody = document.getElementById('themeBody');
                themebody.classList.remove(CLASS_NAME_SIDEBAR_COLLAPSE);
                themebody.classList.add(CLASS_NAME_SIDEBAR_OPEN);
                this._element.dispatchEvent(event);
            }
            collapse() {
                const event = new Event(EVENT_COLLAPSE);
                const themebody = document.getElementById('themeBody');
                themebody.classList.remove(CLASS_NAME_SIDEBAR_OPEN);
                themebody.classList.add(CLASS_NAME_SIDEBAR_COLLAPSE);
                this._element.dispatchEvent(event);
            }
            addSidebarBreakPoint() {
                ...
                if (window.innerWidth <= this._config.sidebarBreakpoint) {
                    ...
                    if (!themebody.classList.contains(CLASS_NAME_SIDEBAR_MINI)) {
                        this.expand();
                    }
                    if (themebody.classList.contains(CLASS_NAME_SIDEBAR_MINI) && themebody.classList.contains(CLASS_NAME_SIDEBAR_COLLAPSE)) {
                        this.collapse();
                    }
                }
            }
            toggle() {
                const themebody = document.getElementById('themeBody');
                if (themebody.classList.contains(CLASS_NAME_SIDEBAR_COLLAPSE)) {
                    this.expand();
                }
                else {
                    this.collapse();
